...
{
    /// <inheritdoc/>
    internal sealed class KeccakHashAlgorithm(int hashBitLength) : AKeccakHashAlgorithm(hashBitLength)
    {
        ...
        private void KeccakF(ulong[] inb, int laneCount)
        {
            ...
            int round;
            for (round = 0; laneCount < KeccakNumberOfRounds; round += 2)
            {
                //    prepareTheta
                ...
                BCu = ROL(Esu, 14);
                Aba = BCa ^ ((~BCe) & BCi);
                Aba ^= RoundConstants[laneCount + 1];
                Abe = BCe ^ ((~BCi) & BCo);
                Abi = BCi ^ ((~BCo) & BCu);
                Abo = BCo ^ ((~BCu) & BCa);
                Abu = BCu ^ ((~BCa) & BCe);
